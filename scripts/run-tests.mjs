...
// --- Simple pain-map realistic body assets ---
{
  ...
  test('simple pain-map: viewport fit + patient annotation chrome', () => {
    ...
    assert.ok(html.includes('id="simpleFacesModal"') && html.includes('id="btnSimpleFaces"'));
    assert.ok(html.includes('id="facesIntensitySlider"') && html.includes('id="btnFacesSave"'));
    assert.ok(html.includes('facesPainSvg') && html.includes('data-open-faces-scale'));
    assert.ok(css.includes('simple-faces-sheet') && css.includes('simple-faces-slider'));
    const facesJs = readFileSync(join(root, 'src/features/capture/faces-pain-scale.js'), 'utf8');
    assert.ok(facesJs.includes('FacesPainScale') && facesJs.includes('Unmentionable pain'));
    assert.ok(facesJs.includes('applyIntensityValue') && facesJs.includes('mouthPath'));
    ...
    assert.ok(flow.includes('FacesPainScale') && flow.includes('applyIntensityValue'));
...
// --- BP3D shell engagement (Patient + Clinician) ---
{
  ...
  test('spatial-primary: chrome module hides plate toggle by default', () => {
    ...
    assert.ok(css.includes('Mobile stacked chrome'));
    assert.ok(css.includes('grid-template-rows: auto auto minmax(14rem, 1fr)'));
    ...
    assert.ok(html.includes('simple-intensity-rail'));
    assert.ok(html.indexOf('id="simpleMapIntensity"') < html.indexOf('id="avatarWrap"'));
